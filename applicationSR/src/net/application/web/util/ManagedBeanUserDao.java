...
public class ManagedBeanUserDao implements UserDao,Serializable{
	...
	public List<CustomerIdmUserCombination> findUsers(
			IdentityQueryBuilder builder, List<Condition> conditionList, Sort sort, Integer offset,	Integer pageSize) {
    	...
    	for (User user : resultUser) {
    		...
    		RelationshipQuery<Grant> queryG = relationshipManager.<Grant> createRelationshipQuery(Grant.class);
        	queryG.setParameter(Grant.ASSIGNEE, user);
        	List<Grant> resultGrant = queryG.getResultList();
	        ...
	        String dummy="dummy";
	    	CIC.setCity(dummy);
	    	CIC.setCountry(dummy);
    		CIC.setDailNumber(dummy);
    		CIC.setEmail(dummy);
    		CIC.setFirstName(dummy);
    		CIC.setGender(dummy);
    		CIC.setLastName(dummy);
    		CIC.setStreet(dummy);
    		CIC.setStreetNumber(dummy);
    		CIC.setUserName(dummy);
    		CICs.add(CIC);
    	}
	...
	public Integer usersCount(IdentityQueryBuilder builder, List<Condition> conditionList, Sort sort) {
		...
		log.info("usersCount:   " + query.getResultCount());
